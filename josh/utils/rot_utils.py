# modified from https://github.com/Arthur151/ROMP/blob/4eebd3647f57d291d26423e51f0d514ff7197cb3/trace/lib/utils/rot_6D.py
...
def positional_encoding_2d(coordinates, C):
    B, N, _ = coordinates.shape
    assert C % 4 == 0, "C must be divisible by 4 for separate sine/cosine encoding of each dimension."

    # Generate a series of frequencies
    d = C // 4
    freqs = torch.pow(10000, torch.arange(0, d, dtype=torch.float32) / d).unsqueeze(0).unsqueeze(0).to(coordinates.device)

    # Positional encodings for x and y
    x_enc = coordinates[..., [0]] / freqs
    y_enc = coordinates[..., [1]] / freqs

    # Concatenate sine and cosine encodings for x and y
    enc_x = torch.cat((torch.sin(x_enc), torch.cos(x_enc)), dim=-1)
    enc_y = torch.cat((torch.sin(y_enc), torch.cos(y_enc)), dim=-1)

    # Concatenate the x and y encodings to form the final encoding
    enc = torch.cat((enc_x, enc_y), dim=-1)
...
def depth_to_3d(depth_image, intrinsics):
    ...
    pixels_homogeneous = np.stack((x + 0.5, y + 0.5, ones), axis=-1).reshape(-1, 3)
    rays = pixels_homogeneous
    rays[:, 0] = (rays[:, 0] - intrinsics[0, 2]) / intrinsics[0, 0]
    rays[:, 1] = (rays[:, 1] - intrinsics[1, 2]) / intrinsics[0, 0]
    z_values = depth_image.reshape(-1)
    points_3d = rays * z_values[:, np.newaxis]
    return points_3d.reshape(h, w, 3)


def create_occupancy_grid(points_3d, grid_size, grid_points, use_occ_only=False):
    occupancy_grid = torch.zeros((grid_points[0], grid_points[1], grid_points[2])).float()
    occupancy_grid[...] = -1
    ...
    voxel_size = torch.Tensor([voxel_size_x, voxel_size_y, voxel_size_z])
    grid_lower_boud = torch.Tensor([grid_size[0], grid_size[2], grid_size[4]])

    # Populate the occupancy grid

    indices = (points_3d[:, :3] - grid_lower_boud.unsqueeze(0)) // voxel_size.unsqueeze(0)
    ...
    if use_occ_only:
        ...
        occupancy_grid[indices[:, 0], indices[:, 1], indices[:, 2]] = points_3d[:, 3]

    return occupancy_grid


def create_3d_grid(grid_size, num_points):
    voxel_size_x = (grid_size[1] - grid_size[0]) / num_points[0]
    voxel_size_y = (grid_size[3] - grid_size[2]) / num_points[1]
    voxel_size_z = (grid_size[5] - grid_size[4]) / num_points[2]
    x = np.arange(grid_size[0] + voxel_size_x / 2, grid_size[1], voxel_size_x)
    y = np.arange(grid_size[2] + voxel_size_y / 2, grid_size[3], voxel_size_y)
    z = np.arange(grid_size[4] + voxel_size_z / 2, grid_size[5], voxel_size_z)
...
def create_2d_grid(grid_size, num_points):
    voxel_size_x = (grid_size[1] - grid_size[0]) / num_points[0]
    voxel_size_z = (grid_size[5] - grid_size[4]) / num_points[2]
    x = np.arange(grid_size[0] + voxel_size_x / 2, grid_size[1], voxel_size_x)
    z = np.arange(grid_size[4] + voxel_size_z / 2, grid_size[5], voxel_size_z)
...
def rotation_6d_to_matrix_numpy(d6: np.ndarray) -> np.ndarray:
    """
    ...
    a1, a2 = d6[..., :3], d6[..., 3:]
    a1_norms = np.linalg.norm(a1, axis=-1, keepdims=True)
    b1 = a1 / a1_norms
...
def interpolate_se3(poses, times, query_times):
    """
    Interpolate SE(3) poses.
    
    :param poses: List of SE(3) poses as (4, 4) numpy arrays
    :param times: List of times corresponding to each pose
    :param query_times: List of times at which to interpolate the pose
    :return: Interpolated SE(3) poses at query_times
    """
    ...
    interp_translation = interp1d(times, translations, axis=0)
    interpolated_translations = interp_translation(query_times)

    # Interpolate rotations using slerp
    slerp = Slerp(times, rotations)

    interpolated_rotations = slerp(query_times).as_matrix()

    # Combine interpolated translations and rotations into SE(3) poses
    interpolated_poses = []
    for translation, rotation in zip(interpolated_translations, interpolated_rotations):
        pose = np.eye(4)
        pose[:3, :3] = rotation
...
def interpolate_so3(rotations, times, query_times):
    """
    Interpolate SO(3) rotations.
    
    :param rotations: List of SO(3) rotations as (3, 3) numpy arrays
    :param times: List of times corresponding to each rotation
    :param query_times: List of times at which to interpolate the rotation
    :return: Interpolated SO(3) rotations at query_times
    """
...
def pixel_to_world(depth_map, K, Tcw):
    ...
    cam_coords_hom = torch.concatenate((cam_coords, torch.ones((cam_coords.shape[0], cam_coords.shape[1], 1))), axis=-1)

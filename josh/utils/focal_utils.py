...
def focal_to_intrinsics(focal, height, width):
    ...
    intrinsics = torch.zeros((*focal.shape, 3, 3), dtype=torch.float32).to(focal.device)
    intrinsics[..., 0, 0] = ff
    intrinsics[..., 1, 1] = ff
...
def recover_focal_shift(
    ...
    progress = range(points.shape[0])
    progress = tqdm(progress, desc="Estimating focal length from local point cloud", leave=False) if verbose else progress
    for i in progress:

...
def local_align_joints(gt_joints, pred_joints):
    ...
    pred_loc = (s_loc[:, None] * torch.einsum("tij,tnj->tni", R_loc, pred_joints) + t_loc[:, None])
    return pred_loc
...
def compute_foot_distance(target_output, pred_output, masks, gt_points, thr=1e-2):
    ...
    distance = (pred_feet_loc.unsqueeze(1).cuda() - gt_points.unsqueeze(0).cuda()).norm(2, dim=-1)
...
def compute_contact_f1(target_output, pred_output, masks, thr=1e-2):
    """compute foot sliding error
    The foot ground contact label is computed by the threshold of 1 cm/frame
    Args:
        target_output (SMPL ModelOutput).
    ...
    pred_feet_loc = pred_output.vertices[:, foot_idxs]
    pred_disp = (pred_feet_loc[1:] - pred_feet_loc[:-1]).norm(2, dim=-1)
    pred_contact = pred_disp[:] < thr
    tp = (contact & pred_contact).sum().item()
    fp = (~contact & pred_contact).sum().item()
    fn = (contact & ~pred_contact).sum().item()
...
def compute_ate(target_trans, pred_trans):
    # Compute the global alignment
    _, rot, trans = align_pcl(target_trans[None, :], pred_trans[None, :], fixed_scale=True)
    pred_trans_hat = (torch.einsum("tij,tnj->tni", rot, pred_trans[None, :]) + trans[None, :])[0]

    # Compute absolute root-translation-error (RTE)
    ate = torch.norm(target_trans - pred_trans_hat, 2, dim=-1)
    ate = torch.sqrt(torch.mean(ate**2)).item()
    return ate
...
def eval_slam(traj_est, traj_gt, return_traj=True, correct_scale=False, align=True, align_origin=False):
    """ Evaluation for SLAM """
    tstamps = np.array([i for i in range(len(traj_est))], dtype=np.float32)

    traj_est = PoseTrajectory3D(poses_se3=traj_est.copy(), timestamps=tstamps)

    traj_ref = PoseTrajectory3D(poses_se3=traj_gt.copy(), timestamps=tstamps)

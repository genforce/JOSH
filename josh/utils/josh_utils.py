# Adapted from MASt3R-SfM (https://github.com/naver/mast3r/blob/main/mast3r/cloud_opt/sparse_ga.py)
import os
...
from dust3r.utils.geometry import geotrf, inv  # noqa
...
from josh.utils.rot_utils import quaternion_to_matrix


class SparseGA():

    def __init__(self, img_paths, pairs_in, res_fine, anchors, canonical_paths=None, smpl_transforms=None):
        ...
        self.smpl_transforms = smpl_transforms
    ...
    def get_dense_pts3d(self, clean_depth=True, subsample=8):
        ...
        pts3d, depthmaps = make_pts3d(anchors, self.intrinsics, self.cam2w, [d.ravel() for d in self.depthmaps], base_focals=base_focals, ret_depth=True)
    ...
    def show(self, show_cams=True):
        ...
        show_reconstruction(self.imgs,
                            ...
                            self.cam2w, [p.clip(min=-50, max=50) for p in pts3d],
...
def sparse_global_alignment(imgs,
    ...
    # forward pass
    pairs, cache_path = forward_mast3r(pairs_in, model, cache_path=cache_path, subsample=subsample, desc_conf=desc_conf, device=device)
    ...
    imgs, res_coarse, res_fine = sparse_scene_optimizer(imgs,
                                                        subsample,
                                                        imsizes,
                                                        pps,
                                                        base_focals,
...
def sparse_scene_optimizer(
    ...
    all_init_quat = [q.detach().clone() for q in smpl_quat]
    all_init_trans = [t.detach().clone() for t in smpl_trans]
    all_init_pose = [p.detach().clone() for p in smpl_body_pose]
    all_init_shape = [s.mean(dim=0).detach().clone() for s in smpl_body_shape]
    smpl_transforms = []
    for q, t, p, s in zip(smpl_quat, smpl_trans, smpl_body_pose, smpl_body_shape):
        smpl_transform = [
            nn.Parameter(q.detach().clone()),
            nn.Parameter(t.detach().clone()),
            nn.Parameter(p.detach().clone()),
            nn.Parameter(s.mean(dim=0).detach().clone())
        ]
        smpl_transforms.append(smpl_transform)

    smpl = SMPL(model_path="data/smpl").to(device)
    ...
    init_focal = log_focals[0].exp().clone().detach()
    enforce_scale = len(smpl_scale_info) == 0
    ...
    def make_K_cam_depth(log_focals, pps, trans, quats, log_sizes, core_depth):
        ...
        cam2w = torch.cat((torch.cat((tmp_cam2w[:, :3, :3], new_trans), dim=2), vec0001.view(1, 1, 4).expand(len(K), 1, 4)), dim=1)
    ...
    if lora_depth:
        core_depth, lora_depth_proj = spectral_projection_of_depthmaps(imgs, K, core_depth, subsample, cache_path=cache_path, **lora_depth)
    if exp_depth:
    ...
    def loss_scale_opt_corres(K, depthmaps, smpl_scale_info_entries, loss_s, pred_smpl):
        """Dynamically reproject SMPL vertices and compare depth against current optimized depthmaps."""
        loss = 0.
        cf_sum = 0.

        for entry in smpl_scale_info_entries:
            image_frame = entry['image_frame']
            smpl_idx_val = entry['smpl_idx']
            vertex_id = entry['vertex_id']

            mask = canon_masks[image_frame]
            H1, W1 = mask.shape
            yx = np.mgrid[subsample // 2:H1:subsample, subsample // 2:W1:subsample]
            H2, W2 = yx.shape[1:]

            # Get current vertex position from SMPL (in camera space)
            vertex_3d = pred_smpl.vertices[smpl_idx_val, vertex_id]  # (3,)
            with torch.no_grad():
                # Project to 2D using current K
                proj = K[image_frame] @ vertex_3d  # (3,)
                if proj[2] <= 0:
                    continue
                uu = proj[0] / proj[2]  # x
                vv = proj[1] / proj[2]  # y
                ...
                # Bounds check
                uu_int = int(uu.detach().round().item())
                vv_int = int(vv.detach().round().item())
                if uu_int < 0 or uu_int >= W1 or vv_int < 0 or vv_int >= H1:
                    continue
                # Skip if projected onto valid (non-human) region already
                if mask[vv_int, uu_int]:
                    continue

                # Find closest valid scene point
                closest = find_closest_valid(mask, torch.tensor(vv_int, device=device), torch.tensor(uu_int, device=device))
                if closest is None:
                    continue
                px, py = closest  # (x, y) in image coords

                # Compute core_idx into the subsampled depthmap
                core_idx = (py // subsample) * W2 + (px // subsample)

                # compute relative depth offsets w.r.t. anchors
                ref_z = canon_core_depths[image_frame][core_idx]
                pts_z = canon_depths[image_frame][py, px]
                human_z = canon_depths[image_frame][vv_int, uu_int]

            # Get scene depth from current optimized depthmap at this core anchor
            scene_depth = depthmaps[image_frame][core_idx] * pts_z / ref_z
            # SMPL vertex depth (z in camera space), scaled by focal ratio
            smpl_depth = vertex_3d[2] * K[image_frame, 0, 0] / init_focal

            # Confidence weight from the precomputed conf map
            conf_weight = float(canon_confs[image_frame][py, px])
            cf_sum += conf_weight
            loss += loss_s(scene_depth, smpl_depth) * conf_weight

        return loss / cf_sum if cf_sum != 0. else 0.

    def loss_scale(K, depthmaps, smpl_scale_info, loss_s, pred_smpl):
        loss = 0.
        cf_sum = 0.

        for image_frame, core_idx, offset, conf, smpl_idx, vertex_idx in smpl_scale_info:
            cf_sum += conf
            loss += loss_s(
                depthmaps[image_frame.long()][core_idx.long()] * offset,
                pred_smpl.vertices[smpl_idx.long(), vertex_idx.long(), 2] * K[0, 0, 0] / init_focal,
            ) * conf

        return loss / cf_sum if cf_sum != 0. else 0.

    def loss_static(pred_smpl, cam2w, smpl_static_info, smpl_img_idx, loss_s):
        loss = 0.
        verts_cam = pred_smpl.vertices
        verts_cam = torch.cat((verts_cam, torch.ones_like(verts_cam[:, :, :1])), dim=-1)
        verts_glob = torch.einsum('bij, bnj -> bni', cam2w[smpl_img_idx], verts_cam)
        verts_glob = verts_glob[:, :, :3]
        for smpl_idx, vertex_idx in smpl_static_info:
            loss += loss_s(verts_glob[smpl_idx.long(), vertex_idx.long(), :], verts_glob[smpl_idx.long() - 1, vertex_idx.long(), :])
        return loss / len(smpl_static_info) if len(smpl_static_info) != 0 else 0.

    def loss_prior(idx, loss_s):
        quat_coeff = 1
        trans_coeff = 1
        pose_coeff = 1
        shape_coeff = 1
        ploss = 0.
        ploss += quat_coeff * loss_s(all_init_quat[idx].flatten(1), smpl_transforms[idx][0].flatten(1)).mean()
        ploss += trans_coeff * loss_s(all_init_trans[idx], smpl_transforms[idx][1]).mean()
        ploss += pose_coeff * loss_s(all_init_pose[idx].flatten(1), smpl_transforms[idx][2].flatten(1)).mean()
        ploss += shape_coeff * loss_s(all_init_shape[idx], smpl_transforms[idx][3]).mean()
        return ploss

    def loss_smooth(pred_smpl, cam2w, smpl_img_idx, loss_s):
        if len(smpl_img_idx) <= 2:
            return 0.
        smpl_img_idx = torch.tensor(smpl_img_idx, device=device)
        joints_cam = pred_smpl.joints[:, :24]
        joints_cam = torch.cat((joints_cam, torch.ones_like(joints_cam[:, :, :1])), dim=-1)
        joints_glob = torch.einsum('bij, bnj -> bni', cam2w[smpl_img_idx], joints_cam)
        joints_glob = joints_glob[:, :, :3]
        joints_glob_2 = joints_glob[2:, :, :]
        joints_glob_1 = joints_glob[1:-1, :, :]
        joints_glob_0 = joints_glob[:-2, :, :]
        smpl_img_idx_2 = smpl_img_idx[2:]
        smpl_img_idx_1 = smpl_img_idx[1:-1]
        smpl_img_idx_0 = smpl_img_idx[:-2]

        loss = loss_s((joints_glob_2 - joints_glob_1) / (smpl_img_idx_2 - smpl_img_idx_1)[:, None, None],
                      (joints_glob_1 - joints_glob_0) / (smpl_img_idx_1 - smpl_img_idx_0)[:, None, None]).mean()
        return loss
    ...
    def optimize_loop(loss_func, lr_base, niter, pix_loss, lr_end=0):
        ...
        if optimize_smpl:
            for smpl_transform in smpl_transforms:
                params += smpl_transform
        ...
        with tqdm(total=niter) as bar:
            for iter in range(niter or 1):
                ...
                pred_smpls = []
                for smpl_transform in smpl_transforms:
                    pred_smpl = smpl(body_pose=smpl_transform[2],
                                     global_orient=quaternion_to_matrix(smpl_transform[0]),
                                     betas=smpl_transform[3].unsqueeze(0).repeat(len(smpl_transform[2]), 1),
                                     transl=smpl_transform[1],
                                     pose2rot=False,
                                     default_smpl=True)
                    pred_smpls.append(pred_smpl)
                ...
                total_frames = 0
                for i, pred_smpl in enumerate(pred_smpls):
                    current_frame = len(smpl_img_idx[i])
                    if opt_corres:
                        loss_scale_value += current_frame * loss_scale_opt_corres(K, depthmaps, smpl_scale_info[i], loss_s, pred_smpl)
                    else:
                        loss_scale_value += current_frame * loss_scale_w * loss_scale(K, depthmaps, smpl_scale_info[i], loss_s, pred_smpl)
                    loss_prior_value += current_frame * loss_prior_w * loss_prior(i, loss_s)
                    loss_static_value += current_frame * loss_static_w * loss_static(pred_smpl, cam2w, smpl_static_info[i], smpl_img_idx[i], loss_s)
                    loss_smooth_value += current_frame * loss_smooth_w * loss_smooth(pred_smpl, cam2w, smpl_img_idx[i], loss_s)
                    total_frames += current_frame
                loss_scale_value /= total_frames
                loss_prior_value /= total_frames
                loss_smooth_value /= total_frames
                loss_static_value /= total_frames
                loss += loss_scale_value + loss_static_value + loss_prior_value + loss_smooth_value
                ...
                if loss != loss:
                    break  # NaN loss
                bar.set_postfix_str(
                    f'{lr=:.4f}, {loss=:.3f}, {loss_scale_value=:.3f}, {loss_static_value=:.3f}, {loss_prior_value=:.3f}, {loss_smooth_value=:.3f}')
                bar.update(1)

        if niter:
            print(f'>> final loss = {loss}')
        return dict(intrinsics=K.detach(), cam2w=cam2w.detach(), depthmaps=[d.detach() for d in depthmaps], pts3d=[p.detach() for p in pts3d])
    ...
    if niter2:
        ...
        for i, img in enumerate(imgs):
            if init[img].get('freeze', 0) >= 1:
                continue
            pps[i].requires_grad_(False)
            log_focals[i].requires_grad_(opt_pp)
    ...
    return imgs, res_coarse, res_fine, smpl_transforms if optimize_smpl else None
...
def make_dense_pts3d(intrinsics, cam2w, depthmaps, canonical_paths, subsample, device='cuda'):
    ...
    # densify sparse depthmaps
    pts3d, depthmaps_out = make_pts3d(anchors, intrinsics, cam2w, [d.ravel() for d in depthmaps], base_focals=base_focals, ret_depth=True)

    return pts3d, depthmaps_out, confs


@torch.no_grad()
def forward_mast3r(pairs, model, cache_path, desc_conf='desc_conf', device='cuda', subsample=8, **matching_kw):
    res_paths = {}
    ...
    for img1, img2 in tqdm(pairs):
        ...
        if not all(os.path.isfile(p) for p in (path1, path2, path_corres)):
            ...
            matching_score = (float(conf_score), float(corres[2].sum()), sum(corres[2] > 0))
...
@torch.no_grad()
def prepare_canonical_data(imgs, tmp_pairs, subsample, pred_points, pred_conf, order_imgs=False, min_conf_thr=1e-3, cache_path=None, device='cuda', **kw):
    canonical_views = {}
    ...
    for idx, img in tqdm(enumerate(imgs)):
        ...
        for (img1, img2), ((path1, path2), path_corres) in tmp_pairs.items():
            ...
            if score is not None:
                ...
                confs11[n] = C
                n += 1
        if pred_points is not None and pred_conf is not None:
            ptmaps11 = pred_points[idx].unsqueeze(0).repeat(n_pairs, 1, 1, 1)
...
PairOfSlices = namedtuple('ImgPair', 'img1, slice1, pix1, anchor_idxs1, img2, slice2, pix2, anchor_idxs2, confs, confs_sum')
...
def condense_data(imgs, tmp_paths, canonical_views, preds_21, dtype=torch.float32):
    ...
    for img1, img2 in tmp_paths:
        ...
        imgs_slices.append(PairOfSlices(img1, slice1, pix1, anchor_idxs1, img2, slice2, pix2, anchor_idxs2, confs, float(confs.sum())))
...
def spectral_projection_depth(K, depthmap, subsample, k=64, cache_path='', normalized_cuts=True, gamma=7, min_norm=5):
    try:
        if cache_path:
...
def spectral_projection_of_depthmaps(imgs, intrinsics, depthmaps, subsample, cache_path=None, **kw):
    ...
    for i, img in enumerate(tqdm(imgs)):
        ...
        depth, proj = spectral_projection_depth(intrinsics[i], depthmaps[i], subsample, cache_path=cache, **kw)
...
@torch.no_grad()
def forward_mast3r_w_mask(pairs, mask_pairs, model, cache_path, desc_conf='desc_conf', device='cuda', subsample=8, **matching_kw):
    res_paths = {}

    for (img1, img2), (mask1, mask2) in tqdm(zip(pairs, mask_pairs), total=len(pairs)):
        idx1 = hash_md5(img1['instance'])
        idx2 = hash_md5(img2['instance'])
        ...
        if not all(os.path.isfile(p) for p in (path1, path2, path_corres)):
            ...
            # apply masking on the confidence
            C11[mask1["img"][0][0] == 1] = 1.0
            C21[mask2["img"][0][0] == 1] = 1.0
            C22[mask2["img"][0][0] == 1] = 1.0
            C12[mask1["img"][0][0] == 1] = 1.0

            qonfs[0][mask1["img"][0][0] == 1] = 0.0
            qonfs[1][mask2["img"][0][0] == 1] = 0.0
            qonfs[2][mask2["img"][0][0] == 1] = 0.0
            qonfs[3][mask1["img"][0][0] == 1] = 0.0
